...
### IMPL-019: Complete Backend Rebuild — Schema, Models, Services, API

- **Rationale:** The `TestingDatabase/testingdatabase.sql` contains the authoritative schema for the lottery system — the source of truth. The current backend had drifted significantly: wrong column types (INTEGER vs REAL for commission rates/jp_factor/sp_factor), wrong table structures (Sale had direct `draw_id`/`agent_id` instead of deriving from batch), wrong field names (`type` vs `restriction_type`/`prize_type` in blacklist/winning tickets), missing settlement tables (5 new tables for draw settlement persistence), missing views (agent sales live, master exposure live, ticket exposure live with risk levels), and a report calculation that didn't match the `CalculationWorkflow.md` formulas (commission was integer-based, payout used wrong factor application, grand total formula had sign errors on dealer cash flows).
- **Changes:**
  - **`database/models.py`:** Complete rewrite — 14 ORM models matching TestingDatabase exactly. Draw: `draw_name` + `opened_at`/`closed_at`/`settled_at` DATETIME fields (replaced `open_date`/`cutoff_time` strings). Agent/MasterDealer: `commission_rate REAL`, `jp_factor REAL`, `sp_factor REAL` (were INTEGER), added `active` flag, removed `note`. Batch: added `batch_no TEXT`, `ticket_count`, `closed_at`, `remarks` (was `note`). Sale: only `batch_id` column (draw and agent derived from batch relationship), `UNIQUE(batch_id, ticket)` — one ticket per batch. Offloaded: `page_no TEXT` (was INTEGER), `notes` (was `note`). BlacklistTicket: `restriction_type` (was `type`). WinningTicket: `prize_type` with JACKPOT/MINOR (was `type` with Jackpot/Minor). NEW tables: `DrawTicketSnapshot`, `DrawSettlementAgent`, `DrawSettlementMaster`, `DrawSettlementTicket`, `DrawSettlementSummary`. Kept `Preference` (not in TestingDatabase but needed for theme persistence).
  - **`database/views.sql`:** Rewrote with 5 views: `v_agent_sales_live` (per-agent sales + commission using REAL commission_rate), `v_master_exposure_live` (per-dealer offloads + commission), `v_ticket_exposure_live` (per-ticket risk with admin_hold, pending, and CRITICAL/HIGH/MEDIUM/LOW risk levels from TestingDatabase), plus current-draw variants.
  - **`database/connection.py`:** Added `_install_triggers()` — installs 3 SQL triggers for batch `total_amount` + `ticket_count` auto-maintenance on sale insert/update/delete. Changed from splitting on `;` to using `cursor.executescript()` (trigger bodies contain semicolons). Same fix applied to `_install_views()`.
  - **`repositories/*` (9 files):** Updated all repositories for new field names. `SaleRepository` now joins through Batch for draw-level queries (`get_by_draw`, `get_ticket_totals`, `get_sales_grouped_by_agent`). `AgentRepository` and `MasterDealerRepository` added `get_active()` methods. `BlacklistRepository` filters on `restriction_type`. `WinningRepository` filters on `prize_type`.
  - **`services/draw_service.py`:** Uses `draw_name` parameter, sets `opened_at`/`closed_at`/`settled_at` on status transitions. Removed date/cutoff string validation.
  - **`services/agent_service.py` + `master_dealer_service.py`:** `commission_rate REAL`, removed `note` field, added `active` flag support.
  - **`services/sales_service.py`:** `record_sale()` takes only `batch_id` + `ticket` + `amount` (draw/agent derived from batch). Removed cutoff time logic. Batch auto-generates `batch_no` as sequential number for the draw.
  - **`services/offload_service.py`:** `page_no` is TEXT. `TicketRisk` named tuple includes `risk_level`. Risk classification thresholds: CRITICAL >= 100000, HIGH >= 50000, MEDIUM >= 10000, else LOW.
  - **`services/report_service.py`:** Complete rewrite using CalculationWorkflow.md formulas. Commission = `amount × commission_rate / 100` (REAL rate). Payout = `amount × factor × half_flag` (REAL factors). Grand total from admin cash-flow perspective: `+subtotal_sales - agent_payout_total - subtotal_offloads + dealer_payout_total`. Added `settle_and_persist()` method that populates all 5 settlement tables. Per-ticket admin_profit_loss = `total_sold - agent_settlement + master_recovery - total_offloaded`. Verified against example: grand total = -9,963,000 (exact match to CalculationWorkflow.md).
  - **`services/blacklist_service.py`:** Uses `restriction_type` parameter name.
  - **`services/winning_service.py`:** Uses `prize_type` with JACKPOT/MINOR values.
  - **`services/risk_service.py`:** Updated to query `v_ticket_exposure_live` view with risk_level aggregation.
  - **`api.py`:** Complete rewrite — all endpoints updated for new field names (camelCase JSON keys: `drawName`, `commissionRate`, `restrictionType`, `prizeType`, `openedAt`/`closedAt`/`settledAt`, etc.). `record_sale()` takes `batch_id` instead of `draw_id`+`agent_id`. `create_offload()` accepts TEXT `page_no`. Risk breakdown includes `riskLevel` per ticket.
  - **`tests/test_models.py` + `tests/test_repositories/test_base.py`:** Updated for new model constructors and field names. Added settlement table test. All 24 tests pass.
- **Files:** 22 files changed across backend/database/ (3), backend/repositories/ (9), backend/services/ (8), backend/api.py, tests/ (2). See `CURRENT_TASK.md` for full details.
- **Verification:** Full business flow tested — agents, dealers, batches, sales, offloads, blacklist, winning tickets, report generation. Report grand total matches CalculationWorkflow.md example exactly. All 24 pytest tests pass.
