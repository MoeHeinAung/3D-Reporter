...
from backend.database.models import Sale, Batch
...
class SaleRepository(BaseRepository[Sale]):
    ...
    def get_by_draw(self, draw_id: int) -> list[Sale]:
        ...
        return list(
            ...
            .filter(Batch.draw_id == draw_id)
            .order_by(Sale.id.desc())
            .all()
        )

    def get_by_draw_and_agent(self, draw_id: int, agent_id: str) -> list[Sale]:
        """Return all sales for a draw+agent combination (via batch join)."""
        return list(
            self.session.query(Sale)
            .join(Batch, Sale.batch_id == Batch.id)
    ...
    def get_sales_grouped_by_agent(self, draw_id: int) -> list[tuple[str, int]]:
        ...
        rows = (
            self.session.query(Batch.agent_id, func.sum(Sale.amount))
            .join(Batch, Sale.batch_id == Batch.id)
            .filter(Batch.draw_id == draw_id)
            .group_by(Batch.agent_id)
            .all()
        )
        return [(str(row[0]), int(row[1])) for row in rows]
